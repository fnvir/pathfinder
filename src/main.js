...
async function Main(){
    // let h=window.innerHeight-document.querySelector('header').offsetHeight,w=window.innerWidth;
    customSelect();
    // wallswitch();
    makegrid();
...
async function startAlgo() {
    ...
    else if(algo=='bellman') await bellmanford(x1,y1,x2,y2);
    //end algo
...
let tr= [-1,1,0,0],tc= [0,0,1,-1];
async function dfs(x1,y1,x2,y2,vis) {
    if(found) return;
    ...
    totalVisited++;
    for(let k=0;k<4;k++){
        let rr=x1+tr[k],cc=y1+tc[k],toVisitNode=rr*M+cc;
...
async function bellmanford(x1,y1,x2,y2){
    ...
    for(let i=0;i<N;i++){
        for(let j=0,r=i*M;j<M;j++) {
            document.querySelector('#col-'+(i*M+j)).classList.add('visited');
            let a=r+j;
            for(let k=0;k<4;k++){
